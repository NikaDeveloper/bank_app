...
@pytest.mark.parametrize(
    ...
    [
        ("Visa 123", "Ошибка: Некорректная длина номера карты"), # Карта, короткий номер
        ("Счет 123", "Ошибка: Некорректная длина номера счета"), # Счет, короткий номер
        ("Card 123456789012345678901", "Ошибка: Некорректная длина номера карты"), # Карта, длинный номер
        ("Счет 12345678901234567890123", "Ошибка: Некорректная длина номера счета"), # Счет, длинный номер
        ("VisaCard", "Ошибка: Некорректный формат входных данных"), # Нет номера
        ("1234567890123456", "Ошибка: Некорректный формат входных данных"), # Только номер
        ("Visa ABCDE12345FGHI6789", "Ошибка: Номер карты/счета должен содержать только цифры"),
...
def test_get_date_empty_string():
    """Проверка, что функция вызывает IndexError при пустой строке."""
    with pytest.raises(IndexError):
        get_date("")
