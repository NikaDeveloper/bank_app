...
def test_filter_by_state_various_states(sample_operations_data, state_to_filter, expected_ids):
...
def test_sort_by_date_descending_default(operations_for_sorting):
    ...
    valid_data = [d for d in operations_for_sorting if "date" in d and d["date"] != "invalid-date-format"]
    result = sort_by_date(valid_data)
    assert [item["id"] for item in result] == [5, 1, 4, 3, 2, 7] # 2020, 2019-07, 2019-07, 2018-09, 2018-06, 2017
...
def test_sort_by_date_ascending(operations_for_sorting):
    ...
    assert [item["id"] for item in result] == [7, 2, 3, 1, 4, 5] # 2017, 2018-06, 2018-09, 2019-07, 2019-07, 2020
